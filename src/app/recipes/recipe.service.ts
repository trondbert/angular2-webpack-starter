...
export class RecipeService {
    ...
    getRecipes(fn) {
        ...
        var recipesRef = RecipeService.getFirebaseRef('recipes/').orderByChild("name");
    ...
    getRecipesByCategory(category, callback) {
        ...
        var recipesRef = RecipeService.getFirebaseRef('recipes/').orderByChild("tags_" + tags).equalTo(true);
    ...
    getRecipe(key:string, fn) {
        ...
        var recipeRef = RecipeService.getFirebaseRef('recipes/' + key);
    ...
    static recipeForStorage(recipe) {
        ...
        var tagsMap = RecipeService.createTagsMap(forStorage.tags);
    ...
    saveRecipeOnly(recipe:Recipe, callback) {
        if (recipe.key) {
            var recipesRef = RecipeService.getFirebaseRef("recipes/");
    ...
    static createTagsMap(tags) {
        var tagsList = RecipeService.createTagsList(tags.split(" ").sort(), 0);
        var tagsMap = {};
    ...
    static createTagsList(tags, index) {
        ...
        var tailTags = RecipeService.createTagsList(tags, index + 1);
